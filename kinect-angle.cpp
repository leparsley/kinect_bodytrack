...
int main()
{
 ...
 /*                   k4a_calibration_3d_to_2d(&sensor_calibration, &P_NOSE.position, K4A_CALIBRATION_TYPE_DEPTH, K4A_CALIBRATION_TYPE_COLOR, &P_NOSE_2D, &result);
                    ...
                    float elbow_angel_right = get_angle(P_SHOULDER_RIGHT.position.xyz.x, P_SHOULDER_RIGHT.position.xyz.y, P_SHOULDER_RIGHT.position.xyz.z, P_ELBOW_RIGHT.position.xyz.x, P_ELBOW_RIGHT.position.xyz.y, P_ELBOW_RIGHT.position.xyz.z, P_WRIST_RIGHT.position.xyz.x, P_WRIST_RIGHT.position.xyz.y, P_WRIST_RIGHT.position.xyz.z);
                    std::cout << "右肘角度;" << elbow_angel_right << std::endl;
                    float elbow_angel_left = get_angle(P_SHOULDER_LEFT.position.xyz.x, P_SHOULDER_LEFT.position.xyz.y,P_SHOULDER_LEFT.position.xyz.z, P_ELBOW_LEFT.position.xyz.x, P_ELBOW_LEFT.position.xyz.y, P_ELBOW_LEFT.position.xyz.z, P_WRIST_LEFT.position.xyz.x, P_WRIST_LEFT.position.xyz.y, P_WRIST_LEFT.position.xyz.z);
